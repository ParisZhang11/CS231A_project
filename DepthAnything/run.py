...
if __name__ == '__main__':
    ...
    depth_anything = DepthAnything.from_pretrained('LiheYoung/depth_anything_{}14'.format(args.encoder))
    ckpt = torch.load('/vision/u/parisz/DepthRegularizedGS/DepthAnything/pretrained/depth_anything_metric_depth_outdoor.pt')
    print(ckpt['model'].keys())
    depth_anything.load_state_dict(ckpt['model'])
    ...
    for filename in tqdm(filenames):
        ...
        if args.pred_only:
            ...
            np.save(os.path.join(args.outdir, filename[:filename.rfind('.')] + '.npy'), depth)
        else:
