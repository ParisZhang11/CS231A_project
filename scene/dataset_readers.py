...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, pcd=None, resolution=4, train_idx=None, white_background=False):
    ...
    for idx, key in enumerate(sorted(cam_extrinsics,key=lambda x:cam_extrinsics[x].name)):
        ...
        # visualization_path = images_folder.replace('images', 'vis_anything') if not use_zoe else images_folder.replace('images', 'vis_zoe')
        obj = images_folder.split('/')[-2]
        # visualization_path = f"/vision/u/parisz/DepthRegularizedGS/output/{obj}_{'anything_2' if not use_zoe else 'depth_2'}/vis"
        # visualization_path = images_folder.replace('nerf_llff_fewshot_resize', 'output')
        # visualization_path = visualization_path + '_anything/vis' if not use_zoe else visualization_path + '_depth/vis'
        ...
        if pcd is not None and idx in train_idx:
            ...
            # source_depth = pipe(image.convert("RGB"))['depth']
            # # source_depth.save(f"depth{idx}.png")
            # source_depth = np.array(source_depth)
            # print('depth-anything', source_depth.max(), source_depth.min())

            # encoder = 'vitl' # can also be 'vitb' or 'vitl'
            # depth_anything = DepthAnything.from_pretrained('LiheYoung/depth_anything_{:}14'.format(encoder)).to('cuda')
            # depth_anything = torch.hub.load('./DepthAnything', 'metric_indoor', source='local').to('cuda')
            # source_depth = depth_anything(image.covert("RGB"))
            # print('depth-anything', source_depth.max(), source_depth.min())
            if not use_zoe:
                metric_depth_path = image_path.replace('images', 'metric_depths').replace('.jpg', '.npy').replace('.png', '.npy')
            # print(image_path, metric_depth_path)
                source_depth = np.load(metric_depth_path)
            # print('depth-anything', source_depth.max(), source_depth.min())
            ...
            p = colorize(source_depth, 0, 10)
            # print(visualization_path, image_name)
            # Image.fromarray(p).save(os.path.join(visualization_path, image_name + '.png'))
            # print('zoe-depth', source_depth2.max(), source_depth2.min())
            # print(source_depth2.shape)
            # im = Image.fromarray(source_depth * 255).astype(np.uint8)
            # im = im.convert("L")
            # im.save(f"depth2_{idx}.png")
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", use_depth=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            print(os.path.exists(os.path.join(path, frame["file_path"].replace("/train/", "/depths_train/")+'.npy')))
