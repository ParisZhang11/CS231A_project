...
def read_poses(images_path):
    ...
    for i, content in enumerate(contents[::2]):
        ...
        img_num = int((img_name.strip().split("."))[0]) - 1 # "00056.jpg\n" -> 56
        poses.append(T)
        idx[img_num] = i
...
if __name__ == "__main__":
    ...
    if args.dset=='nerfllff' or args.dset=='dtu': ## forward-facing
        ...
        train_list = np.random.choice(len(c2ws), int(len(c2ws) * 0.75), replace=False)
        # train_list = [i for i in range(len(c2ws)) if np.random.rand()<0.75]
